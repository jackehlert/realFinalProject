...
while ~isDone 
    ...
    displayBoard(board);
    % Take in input 
    coordinates = input(['Where to place player ' player '?  '], 's');
    %Exampleinputs: 1,1 or 3,2 ; number comma number no spaces
...
if isWon == true 
    ...
    endendfunction [] = displayBoard(board)
    fprintf('   1 2 3 \n');
    fprintf('  +-+-+-+\n'); 
    for i = 1:3   
        newRow = true;
        ...
        fprintf('|\n'); 
        fprintf('  +-+-+-+\n'); 
        endendfunction [isWon,isDone] = checkBoard(board) 
        ...
        if isWon == true || spotsUsed == 9 
            ...
            isDone = false;
            endendfunction isDone = checkWin(board)
